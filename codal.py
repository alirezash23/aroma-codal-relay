...
def match_symbol(symbol: str, name: str, text: str) -> Optional[str]:
    """
    تطبیق نماد در متن آزاد تلگرام — سه لایه، از دقیق‌ترین به شل‌ترین.

    تجربه اولین اجرای واقعی: جستجوی ساده نماد در متن، «خراسان» را داخل
    «#اخشان_خراسان» و «بوعلی» را داخل «#سرمایه_گذاری_بوعلی» مچ می‌کرد و
    اطلاعیه یک شرکت را به شرکت دیگری نسبت می‌داد.

    اما تجربه بعدی نشان داد سخت‌گیری کامل هم اطلاعیه‌های واقعی مرتبط
    (مثلاً هشتگ چندبخشی رسمی خود شرکت) را از دست می‌دهد. تصمیم: هر سه
    لایه تلاش می‌شود، ولی خروجی هرکدام برچسب جدا دارد تا لایه‌های
    غیرقطعی قابل تشخیص و بررسی دستی باشند — چیزی گم نمی‌شود، ولی هرچه
    نامطمئن‌تر است شفاف‌تر علامت می‌خورد.

    لایه‌ها:
    - hashtag: هشتگ دقیق نماد به‌تنهایی (#زاگرس) — قطعی
    - company_name: نام کامل شرکت در متن («پتروشیمی جم») — قطعی
    - loose: خود نماد به‌صورت یک کلمه مستقل در هر جای متن، از جمله
      داخل هشتگ‌های چندبخشی (#مبین_انرژی) — ممکن است گاهی نامرتبط
      بیاورد (مثل «خراسان» داخل «اخشان_خراسان»)، برای همین جدا
      برچسب می‌خورد؛ در سرو نهایی (matched_by) قابل فیلتر است.
    """
    raw = normalize_fa(text)
    flat = re.sub(r"[_\-]+", " ", raw)
    sym = re.escape(normalize_fa(symbol))

    if re.search(rf"#\s*{sym}(?![\w\u200c])", raw):
        return "hashtag"
...
def match_telegram(symbol: str, posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for p in posts:
        ...
        if how == "loose":
            warning += (" تطبیق نامطمئن: نماد در متن پیدا شد ولی نه به‌عنوان "
                        "هشتگ دقیق یا نام کامل شرکت — ممکن است مربوط به یک "
                        "نهاد دیگر با نام مشابه باشد، قبل از استفاده بررسی شود.")
        out.append({
            "matched_by": how,
